...
if __name__== '__main__':
    ...
    if case ==1:
        ...
        rdd_test_final_reduce_output= rdd_case_1.mapPartitions(frequentitems).flatMap(lambda x:x).map(lambda x:(x,1)).reduceByKey(lambda x,y:(x+y)).map(lambda x: x[0]).mapPartitions(map2).map(lambda x:x).collect()
